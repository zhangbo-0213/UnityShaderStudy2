...
//RayMarching is the main scene,ignore unity sky box
void MergeUnityIntoRayMarching(inout float rz,inout float3 rCol,float unityDep,float3 unityCol){
	if(unityDep<rz&&unityDep<_ProjectionParams.z-1.){ //_ProjectionParams.z 为相机远裁剪面在投影空间下距离相机距离
		rCol=unityCol.xyz;
		rz=unityDep;
	}
}
//Unity Scene is the main world,ignore raymarching sky box
void MergeRayMarchingIntoUnity(inout float rz,inout float rCol,float unityDep,float3 unityCol){
	if(unityDep<rz){
		rCol=unityCol.xyz;
		rz=unityDep;
	}
